...
int	process_iodevctl(int sock)
{
	...
	if (params.subcmd == DEVCTL_GET_INFO) {
		memset(&result.getinfo, 0, sizeof(struct DevctlGetInfo));
#ifdef __CYGWIN__
                struct statfs st;

                if(statfs(rootdir, &st) < 0)
                {
                        fprintf(stderr, "Could not stat %s (%s)\n", rootdir, strerror(errno));
                }

                result.getinfo.max_clusters = st.f_blocks;
                result.getinfo.free_clusters = st.f_bfree;
                result.getinfo.max_sectors = st.f_blocks;
                result.getinfo.sector_size = 512;
                result.getinfo.sector_count = st.f_bsize / 512;
#else
                struct statvfs st;

                if(statvfs(rootdir, &st) < 0)
                {
                        fprintf(stderr, "Could not stat %s (%s)\n", rootdir, strerror(errno));
                }
                ...
                result.getinfo.max_sectors = st.f_blocks;
                result.getinfo.sector_size = 512;
                result.getinfo.sector_count = st.f_frsize / 512;
#endif
	}
	result.res = 0;
	SEND_RESULT();
...
int process_cmd(int sock)
{
	...
	if (!g_login) {
		...
		switch (cmd)
		{
			...
			case NET_HOSTFS_CMD_IODEVCTL:
				process_func = process_iodevctl;
			break;
...
int main(int argc, char *argv[])
{
	...
	printf("nethostfs v2.1 starting up%s, maximum of %d PSPs allowed\n", rdonly ? " under read-only mode":"", max_clients);
#ifdef __CYGWIN__
	if (keypad_redirect) {
