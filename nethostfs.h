...
// Only cater for DEVCTL_GET_INFO command
typedef PACKEDSTRUCT IO_DEVCTL_PARAMS
{
	u32		subcmd; // Devctl sub-command
	u32		fs_num; // Ignored at the moment
} IO_DEVCTL_PARAMS;
...
// only cater for DEV_GET_INFO only
typedef PACKEDSTRUCT IO_DEVCTL_RESULT
{
	int res;
	struct DevctlGetInfo getinfo;
} IO_DEVCTL_RESULT;
